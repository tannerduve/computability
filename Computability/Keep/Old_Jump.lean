...
-- def φ (g : ℕ →. ℕ) (e : ℕ) : ℕ →. ℕ :=
--   evalo g (decode' e)
-- def halting_set : ℕ →. Prop := (fun n => n ∈ (φ n n).dom)

def halting_set (n : ℕ) : Prop :=
  n ∈ (φ (fun _ => Part.some 0) n).Dom

/-- The **Halting problem** is not computable -/
-- theorem halting_problem (n) : ¬ComputablePred fun c => (eval c n).Dom
--   | h => rice { f | (f n).Dom } h Nat.Partrec.zero Nat.Partrec.none trivial

def halting_function : ℕ →. ℕ :=
  fun n => φ (fun _ => Part.some 0) n n


-- lemma halting_not_recursive : ¬RecursiveIn (fun _ => Part.some 0) halting_function := by
--   intro H




-- def halt : ℕ →. Prop := (fun n => φ n n )
